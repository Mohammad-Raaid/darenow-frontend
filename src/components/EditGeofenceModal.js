...
const EditGeofenceModal = ({ isOpen, onClose, geofenceData, mallName }) => {
    ...
    useEffect(() => {
        if (isOpen && geofenceData) {
            setFormData({
                geofenceName: geofenceData.geofenceName || '',
                latitude: geofenceData.latitude?.toString() || '',
                longitude: geofenceData.longitude?.toString() || '',
                radiusMeters: geofenceData.radiusMeters?.toString() || '100',
    ...
    const onRadiusChanged = useCallback(() => {
        if (circleRef.current !== null) {
            ...
            setFormData(prev => ({
                ...prev,
                radiusMeters: newRadius.toString()
            }));
        }
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await api.put(`/admin/geofences/${geofenceData.geofenceId}`, payload);
            showToast('Geofence updated successfully!', 'success');
            onClose();
        } catch (error) {
            console.error('Error updating geofence:', error);
            showToast(error.response?.data?.message || 'Failed to update geofence', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all overflow-y-auto">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl transition-all duration-300 overflow-hidden my-8">
                ...
                <div className="bg-gradient-to-r from-[#EB422B] to-[#ff5d47] px-8 py-5">
                    <h3 className="text-xl font-extrabold text-white">Edit Geofence</h3>
                    <p className="text-white/80 text-sm mt-0.5 font-medium">Modify virtual perimeter for {mallName}</p>
                </div>
                            ...
                            <div className={`${isMaximized ? 'bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300' : 'space-y-3'}`}>
                                ...
                                <div className={`${isMaximized ? 'p-6 flex-1 flex flex-col gap-4' : 'space-y-3'}`}>
                                    ...
                                    <div className={`relative border-4 border-gray-50 ${isMaximized ? 'flex-1 rounded-2xl' : 'rounded-2xl h-[300px]'} overflow-hidden shadow-inner group transition-all duration-300`}>
                                        ...
                                        {/* Geofencing Radius Control Overlay */}
                                        <div className="absolute bottom-4 left-4 right-4 z-10 bg-white/95 backdrop-blur-sm p-4 rounded-2xl border border-white shadow-xl max-w-[280px]">
                                            <div className="flex justify-between items-center text-[10px] font-extrabold text-gray-500 uppercase tracking-widest mb-2">
                                                <span>Radius</span>
                                                <span className="text-[#EB422B] text-xs px-2 py-0.5 bg-[#EB422B]/10 rounded-full">{formData.radiusMeters || 0}m</span>
                                            </div>
                                            <input
                                                type="range"
                                                name="radiusMeters"
                                                ...
                                                onChange={handleChange}
                                                className="w-full h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#EB422B]"
                                            />
                                            <div className="flex justify-between mt-1 text-[8px] text-gray-400 font-bold uppercase">
                                                <span>20m</span>
                                                <span className="ml-auto">1km</span>
                                            </div>
                                        </div>
...
//                                                     {formData.polygon?.length || 0} Points
//                                                 </span>
//                                             </div>
